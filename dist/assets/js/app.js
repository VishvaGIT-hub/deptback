...
(function () {
	...
	function analtyicsIntroAnimation() {
		...
		gtag('send', 'event', 'Game', 'Intro', 'Animation');
	}

	function setLanguageOnLoad() {
		if (!$.urlParam('lang')) {
			currentLanguage = '';
			$('.language-selector-text').text("Spanish Version");
		} else if ($.urlParam('lang').toLowerCase() === 'es') {
			currentLanguage = '_es';
			$('body').addClass('langEs');
			$('.language-selector-text').text("English Version");
		} else {
			currentLanguage = '';
			$('.language-selector-text').text("Spanish Version");
		}
	...
	// function checkLanguageButtonText() {
	// 	const buttonText = document.getElementsByClassName('language-selector-text');

	// 	if (currentLanguage === '' || '_en') {
	// 		buttonText.innerHTML = "Spanish Version";
	// 	} else {
	// 		buttonText.innerHTML = "English Version";
	// 	}
	// }
	...
	function getInputs() {
		...
		gtag('send', 'pageview', '/intro-branded');
		gtag('send', 'event', 'Game', 'Intro', 'Logo');
	}
	...
	function goToInputs() {
		...
		gtag('send', 'pageview', '/user-questions');
	}
	...
	function getCollegeResults() { //click apply button on intro form
		...
		var $gpa = $('.gpalist option:checked').val(),
			$state = $('.statelist option:checked').val(),
			$extra = $('.extralist option:checked').val(),
			gpaTxt = $('.gpalist option:checked').text(),
			stateTxt = $('.statelist option:checked').text(),
			extraTxt = $('.extralist option:checked').text();
		...
		if ($gpa == '' || $state == '' || $extra == '') {
			...
			selectedState = setSelectedById(loadedStates, $state);
			//hide error message
		...
		//User prompted to answer GPA, extracurricular and state questions. Only one page view needed, but multiple events triggers are needed.
		gtag('send', 'event', 'User Input', 'GPA', gpaTxt);
		gtag('send', 'event', 'User Input', 'State', stateTxt);
		gtag('send', 'event', 'User Input', 'Extracurriculars', extraTxt);

		gtag('send', 'pageview', '/choose-college');
	}
	...
	function getCollegeId() {
		...
		currentSectionIndex = 0;
		gtag('send', 'event', 'User Input', 'College Path', selectedPathData.selectedPath.label);
		...
		gtag('send', 'event', 'User Input', 'College Name', currentCollege.name);
	}
	...
	function clickToStartFirstQuestion() {
		...
		gtag('send', 'pageview', '/game/highschool/start');
		gtag('send', 'event', 'Highschool', 'Highschool', 'Highschool');
	};
	...
	// 			gtag('send', 'pageview', '/game/' + $sectionLabelUrl + '/challenge/' + $challengeId);
	// 			gtag('send', 'event', $sectionLabel, 'Challenge', $challengeText);
	// 			break;
	// 		case 'message':
	// 			break;
	// 		case 'title':
	// 			gtag('send', 'pageview', '/game/' + $sectionLabelUrl + '/start');
	// 			gtag('send', 'event', $sectionLabel, 'Path', 'Start');
	// 			break;
	// 	}
	// }
	...
	function challengeAnalytics(challenge, outcome) {
		...
		gtag('send', 'pageview', '/game/' + $sectionLabelUrl + '/challenge/' + $challengeId + '/' + $outcomeId);
		gtag('send', 'event', $sectionLabel, 'Outcome', $outcomeLabel);
	}
	...
	function titleAnalytics(section) {
		...
		gtag('send', 'pageview', '/game/' + $titleLabelUrl + '/start');
		gtag('send', 'event', $title, 'Path', 'Start');
	}
	...
	function trackEndScreens() {
		if (gradSchool === true) {
			...
			gtag('send', 'pageview', '/game/outcome/graduate/grad-school');
			gtag('send', 'event', 'Game Over', 'Graduate', 'Grad School');
		} else {
			if (meterConnections <= 250) {
				//user had low connections
				gtag('send', 'pageview', '/game/outcome/graduate/low-connections');
				gtag('send', 'event', 'Game Over', 'Graduate', 'Low Connections');
			} else {
				var $salary = selectedMajor.meanSalary;

				if ($salary < meterLoans) {
					//user had really high debt
					gtag('send', 'pageview', '/game/outcome/graduate/too-much-debt');
					gtag('send', 'event', 'Game Over', 'Graduate', 'Too Much Debt');
				} else {
					//user had a good balance
					gtag('send', 'pageview', '/game/outcome/graduate');
					gtag('send', 'event', 'Game Over', 'Graduate', 'Good Balance');
				}
			}
		}
	}
	...
	function loadFocusEnd($panel) {
		...
		gtag('send', 'pageview', '/game/outcome/dropout/low-focus');
		gtag('send', 'event', 'Game Over', 'Drop Out', 'Low Focus');
		//animate to the next panel
		animatePanel(panelNm, 'down', 0, addCurrent);
	}
	...
	function loadHappinessEnd($panel) {
		...
		gtag('send', 'pageview', '/game/outcome/dropout/low-happiness');
		gtag('send', 'event', 'Game Over', 'Drop Out', 'Low Happiness');
